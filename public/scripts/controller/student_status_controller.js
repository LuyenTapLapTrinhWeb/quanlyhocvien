app.controller('student_status_ctl', ['$scope', '$http', '$window', '$compile', '$timeout', function ($scope, $http, $window, $compile, $timeout) {
    ...
    $scope.addstudent_status = function () {
        ...
        // check name existed
        for (var i = 0; i < $scope.student_status_list.length; i++) {
            if (angular.lowercase($scope.student_status_list[i].name_ss) == angular.lowercase($scope.student_status.name_ss)) {
                $scope.exiss1 = true;
                $timeout(function () {
                    $scope.exiss1 = false;
                }, 3000);
    ...
    $scope.updatestudent_status = function () {
        ...
        //kiem tra trung name
        for (var i = 0; i < $scope.student_status_list.length; i++) {
            if (angular.lowercase($scope.student_status_list[i].name_ss) == angular.lowercase($scope.editstudent_status.name_ss) && $scope.student_status_list[i].id_ss != $scope.editstudent_status.id_ss) {
                $scope.message = 'Update Fail';
                jQuery("#myModalmessage").modal('show');
                refresh();
                $scope.exiss1 = true;
