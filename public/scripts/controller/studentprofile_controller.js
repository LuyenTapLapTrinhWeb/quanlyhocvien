app.controller('studentprofile_ctl', ['$scope', '$http', '$window', '$compile', '$timeout', function ($scope, $http, $window, $compile, $timeout) {
    ...
    $scope.addstudentprofile = function () {
        ...
        // check name existed
        for (var i = 0; i < $scope.studentprofile_list.length; i++) {
            if (angular.lowercase($scope.studentprofile_list[i].name_profile) == angular.lowercase($scope.studentprofile.name_profile)) {
                $scope.exiss1 = true;
                $timeout(function () {
                    $scope.exiss1 = false;
                }, 3000);
    ...
    $scope.updatestudent_profile = function () {
        ...
        // kiem tra trung name
        for (var i = 0; i < $scope.studentprofile_list.length; i++) {
            if (angular.lowercase($scope.studentprofile_list[i].name_profile) == angular.lowercase($scope.editstudentprofile.name_profile) && $scope.studentprofile_list[i].id_profile != $scope.editstudentprofile.id_profile) {
                $scope.message = 'Update Fail';
                jQuery("#myModalmessage").modal('show');
                refresh();
                $scope.exiss1 = true;
