app.controller('lecturer_ctl', ['$scope', '$http', '$window', '$compile', '$timeout','Upload', function ($scope, $http, $window, $compile,$timeout,Upload) {
	...
	var refresh = function () {
		...
		$http({
			method: 'GET',
			url: '/menu_Lecturers_user_add'
		}).then(function successCallback(response) {
			$scope.lecturers_list_user_add = response.data;
		}, function errorCallback(response) {
	...
	var Add = function(){
		...
		for (var i = 0; i < $scope.lecturers_list.length; i++) {
			...
			//set user_code
			for(var i = 0;i< $scope.lecturers_list_user_add.length;i++){
				if($scope.lecturers_list_user_add[i].fullname == $scope.fullname){
					$scope.lecturer.user_code = $scope.lecturers_list_user_add[i].user_code;
					break;
				}
			}
		
	
			// khi lecturers_list_user_add rong
			if($scope.lecturers_list_user_add == null || $scope.lecturers_list_user_add ==""){
				$scope.lecturer_null = true;
				$timeout(function () {
							$scope.lecturer_null = false;
						}, 3000);
						return;
			}
			...
			$scope.full_name = $scope.fullname;
